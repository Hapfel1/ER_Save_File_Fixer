...
@dataclass
class Gestures:
    """All gesture IDs (0x100 = 256 bytes: 64 gestures × 4 bytes)"""
    gesture_ids: List[int] = field(default_factory=list)
    
    @classmethod
    def read(cls, f: BytesIO) -> Gestures:
        """Read Gestures from stream (256 bytes)"""
        obj = cls()
        obj.gesture_ids = [struct.unpack("<I", f.read(4))[0] for _ in range(64)]
        return obj
    
    def write(self, f: BytesIO):
        """Write Gestures to stream (256 bytes)"""
